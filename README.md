...
| [google-closure-compiler](https://github.com/git+https://github.com/google/closure-compiler-npm.git#master) | [20250601.0.0](https://www.npmjs.com/package/google-closure-compiler/v/20250601.0.0) | 2025-06-03     |
...
| 3. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                                             |       <sup>-68% </sup>`22.84 kB` |       <sup>-58% </sup>`8.22 kB` | <sup>*1151x* </sup>`3,024 ms` |
...
| 11. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                                  |       <sup>-68% </sup>`92.73 kB` |       <sup>-61% </sup>`33.09 kB` | <sup>*521x* </sup>`4,105 ms` |
...
| 6. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                          |       <sup>-66% </sup>`115.61 kB` |       <sup>-51% </sup>`44.18 kB` | <sup>*339x* </sup>`4,443 ms` |
...
| 2. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                                    |       <sup>-86% </sup>`73.50 kB` |       <sup>-74% </sup>`25.02 kB` | <sup>*365x* </sup>`4,292 ms` |
...
| 10. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                                     |       <sup>-51% </sup>`270.32 kB` |       <sup>-28% </sup>`94.17 kB` | <sup>*201x* </sup>`6,532 ms` |
...
| 8. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                                          | **<sup>🏆-56% </sup>`439.97 kB`** |       <sup>-35% </sup>`126.45 kB` | <sup>*176x* </sup>`6,136 ms` |
...
| 5. [google-closure-compiler](packages/minifiers/minifiers/google-closure-compiler.ts)                                      |       <sup>-48% </sup>`644.52 kB` |       <sup>-34% </sup>`162.77 kB` |      <sup>*161x* </sup>`8,048 ms` |
...
<!-- analysis:start -->
The crowd’s buzzing with excitement as we analyze the thrilling head-to-head matchups among JavaScript minifiers. It’s a contest of crunching powerhouses where size and speed reign supreme. From milliseconds of blazing fast compression to byte-shaving wizardry, let’s dive into the showdown!

### Best minifier
**@swc/core emerges as the champion!** This minifier showcases an exceptional combination of tiny file sizes and lightning-fast performance, frequently landing at or near the smallest gzip sizes while operating much faster than competitors like uglify-js or terser. Whether it’s squashing `jquery` down to 30.87 kB in just 53 milliseconds or dominating the massive `echarts` package, it consistently strikes an ideal balance between speed and size. With no crashes or timeouts, @swc/core proves itself as the optimal choice for most users who demand both efficiency and effectiveness.

### Honorable mentions
1. **oxc-minify**: Wow! If you need the *fastest* minifier, here’s your pick! It earned the gold medal multiple times for time (like crunching `terser` to 122.35 kB in an absurd 37 ms), but its compression ratios lag just behind heavier competitors like @swc/core or uglify-js. Perfect for projects where speed is priority number one.
2. **uglify-js**: The original heavy-hitter doesn't disappoint, boasting the absolute smallest gzip size in several rounds (e.g., `lodash` at 24.69 kB). However, its processing times are glacial by comparison, regularly clocking in at seconds where others take milliseconds. A true min-maxer’s delight for squeezing every last byte, but definitely not your everyday pick.
3. **@tdewolff/minify**: Consistently the fastest in almost every round, it’s a sprinter that prioritizes processing speed over optimal compression. Ideal for scenarios where shaving milliseconds off your build pipeline is critical, though its file sizes land toward the slightly larger end of the spectrum.

### Eliminated
1. **babel-minify**: Uh-oh, this one crashed during the `d3` round, unable to handle certain modern JavaScript structures. Beware—while promising on paper, its fatal errors make it unreliable for production.
2. **tedivm/jshrink**: A noble effort, but it failed dramatically in the `d3` round due to a regex mishap. Trusting this minifier could lead to corrupted builds. Approach with extreme caution!
3. **bun**: Speedy bun stumbled in post-validation during the `typescript` round, producing inaccurate results. A minifier shouldn’t compromise correctness for performance.

### Final thoughts
What an exhilarating match! With @swc/core leading the pack as the best choice for size, speed, and stability, and oxc-minify making a strong case for instant compression needs, the battle for the optimal minifier has never been more competitive. Let’s give a round of applause to all contenders—you’ve pushed the boundaries of JavaScript minification brilliance!
<!-- analysis:end -->

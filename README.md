...
_Benchmarks last updated on <!-- lastUpdated:start -->May 6, 2025<!-- lastUpdated:end -->._
...
<!-- analysis:start -->
The JavaScript minifiers went head-to-head in a spectacular showdown, flying through tests of size and speed! Let’s unpack this thrilling competition and crown the ultimate winner.

### Best minifier
**@swc/core** takes the crown! This powerhouse blends astonishing compression rates with lightning-fast processing. With standout performances compressing gigantic npm packages like `antd` (55% size reduction in 1,185 ms) and `typescript` (46% size reduction in 2,120 ms), it proves its worth across various use cases. When the stakes get gigantic, @swc/core delivers exceptional results without sacrificing speed.

### Honorable mentions
- **uglify-js**: The veteran showed its mettle with unmatched compression rates, taking first place in six rounds. However, its slower processing times (e.g., 6,579 ms for `victory`) make it more suited to scenarios where size optimization is the sole priority.
- **oxc-minify**: Stellar performance for speed demons. Its near-instantaneous execution (just 3 ms for `react`, 8 ms for `moment`) allows blazing-fast builds without compromising too much on compressed size.
- **@tdewolff/minify**: The speed king! Consistently the fastest minifier, it’s unrivaled for rapid builds, especially in CI pipelines or development environments. While it's often a bit less aggressive on size (e.g., 53% for `moment` vs. 51% for the top contenders), it still punches above its weight.
- **terser**: A balanced performer, especially for large and complex packages. Though slightly slower (e.g., 6,823 ms for `antd`), it maintains competitive compression while often finishing in third or fourth place.
- **esbuild**: Incredibly fast but sacrifices size for performance and simplicity. Great for rapid prototyping or development pipelines.

### Eliminated
- **babel-minify**: Alas! Disqualified for runtime failures, struggling to handle even moderate challenges. It couldn’t keep up with the pace and fell apart during `d3` testing.
- **tedivm/jshrink**: A valiant attempt but floundered due to unclosed regex errors on `d3`. This makes it unreliable for real-world JavaScript codebases.
- **bun**: A surprising misstep during the `typescript` test. Its post-validation errors could spell trouble in production.

### Recap
What a thrilling race! The contestants gave it their all, producing incredible results in both size and speed. While @swc/core reigns supreme for its versatile brilliance, other minifiers like uglify-js, oxc-minify, and @tdewolff/minify each carve out niches where they shine brightest. Honor to all who competed; the bar for JavaScript optimization is higher than ever!
<!-- analysis:end -->
